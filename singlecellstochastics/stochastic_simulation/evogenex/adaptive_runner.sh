...
for data_file in $DATA_PATH*; do
  ...
  if [ "$USE_SLURM" == "true" ]; then
    ...
    sbatch \
      --job-name=$job_name \
      src/adaptive_driver.sbatch $TREE_FILE $SINGLE_REGIME_FILE $TWO_REGIME_FILE $data_file $output_file  
  else
    echo "Using CPU"
    Rscript src/adaptive_evogenex.R $TREE_FILE $SINGLE_REGIME_FILE $TWO_REGIME_FILE $data_file $output_file
  fi
done
