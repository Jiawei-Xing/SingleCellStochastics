...
parser.add_argument("-i", type=int, required=False, default=500, help="Max number of iterations for optimization") # default is 500
...
for batch_start in range(0, len(df.columns), batch_size):
    ...
    for i in range(batch_size): 
        h0_theta = torch.nn.functional.softplus(h0_params[i, 0, -len(regimes)])
        h1_theta = torch.nn.functional.softplus(h1_params[i, 0, -len(regimes):])
