...
def load_scores(oup_file, egx_file, wlx_file):
    ...
    return score_oup, score_egx, score_wlx

'''
def load_scores(oup_file, egx_file, wlx_file):
    """Load and compute absolute -log10(p) scores for each method."""
    df_oup = pd.read_csv(oup_file, sep="\t")
    df_egx = pd.read_csv(egx_file, sep=",")
    df_wlx = pd.read_csv(wlx_file, sep="\t")

    # Use absolute significance, safeguarded against p=0
    score_oup = -np.log10(np.maximum(1e-300, df_oup["p"].values))
    
    # EGX: paired rows (theta0, theta1); p-value from first of each pair
    score_egx = -np.log10(np.maximum(1e-300, df_egx["ou2_vs_ou1_pvalue"].iloc[::2].values))
    
    score_wlx = -np.log10(np.maximum(1e-300, df_wlx["p_value"].values))

    return score_oup, score_egx, score_wlx
'''

def plot_roc_curve(ax, oup_neg, oup_pos, egx_neg, egx_pos, wlx_neg, wlx_pos, title=None):
    s_oup_neg, s_egx_neg, s_wlx_neg = load_scores(oup_neg, egx_neg, wlx_neg)
    s_oup_pos, s_egx_pos, s_wlx_pos = load_scores(oup_pos, egx_pos, wlx_pos)
    ...
    if title:
        ax.set_title(title)
    # Reorder legend: OUP, EvoGeneX, Wilcoxon
...
for i, row in enumerate(datasets):
    for j, (pos, neg) in enumerate(row):
        ...
        plot_roc_curve(axes[i, j], oup_neg, oup_pos, egx_neg, egx_pos, wlx_neg, wlx_pos)

fig.supxlabel("False Positive Rate")
fig.supylabel("True Positive Rate")
plt.tight_layout(pad=0.5)
plt.savefig("diff_ROC2.png", dpi=300, bbox_inches="tight")
plt.show()
